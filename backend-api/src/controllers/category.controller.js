const JSend = require("../jsend");
const ApiError = require("../api-error");
const catchAsync = require("../catchAsync");
const categoryService = require("../services/category.service");

/**
 * Creates a new category.
 * The request body is validated by Zod middleware.
 */
const createCategory = catchAsync(async (req, res, _next) => {
  let { name, url_path, description } = req.body;

  // If url_path is not provided, generate it from the category name.
  if (!url_path && name) {
    url_path = name
      .toLowerCase()
      .replace(/[^a-z0-9 -]/g, "")
      .replace(/\s+/g, "-")
      .replace(/-+/g, "-");
  } else if (url_path) {
    // If provided, ensure it's lowercased and trimmed.
    url_path = url_path.toLowerCase().trim();
  }

  const categoryData = { name, url_path, description };

  const newCategory = await categoryService.createCategory(categoryData);
  return res.status(201).json(JSend.success({ category: newCategory }));
});

/**
 * Updates an existing category.
 * The request body and params are validated by Zod middleware.
 */
const updateCategory = catchAsync(async (req, res, _next) => {
  const { id } = req.params;
  let { name, url_path, description } = req.body;

  // Logic to handle url_path updates.
  if (url_path) {
    // If url_path is explicitly provided, use the new value.
    url_path = url_path.toLowerCase().trim();
  } else if (name !== undefined && url_path === undefined) {
    // If name is updated but url_path is not, regenerate it from the new name.
    url_path = name
      .toLowerCase()
      .replace(/[^a-z0-9 -]/g, "")
      .replace(/\s+/g, "-")
      .replace(/-+/g, "-");
  }
  // If url_path is explicitly set to null or name is not updated, it will be handled accordingly.

  const updateData = {};
  if (name !== undefined) updateData.name = name;
  if (url_path !== undefined) updateData.url_path = url_path;
  if (description !== undefined) updateData.description = description;

  // A Zod schema with .refine() already ensures that at least one field is being updated.
  // This check is a fail-safe.
  if (Object.keys(updateData).length === 0) {
    return _next(new ApiError(400, "No data provided for update."));
  }

  const updatedCategory = await categoryService.updateCategory(id, updateData);
  if (!updatedCategory) {
    return _next(
      new ApiError(404, "Category not found with the specified ID.")
    );
  }
  res.status(200).json(JSend.success({ category: updatedCategory }));
});

/**
 * Retrieves all categories with filtering and pagination.
 * Query parameters are validated by Zod middleware.
 */
const getAllCategories = catchAsync(async (req, res, _next) => {
  const filters = req.query;
  const { categories, totalItems, currentPage, totalPages, limit } =
    await categoryService.getAllCategories(filters);
  res.status(200).json(
    JSend.success({
      categories,
      metadata: {
        totalRecords: totalItems,
        currentPage,
        totalPages,
        limit,
      },
    })
  );
});

/**
 * Retrieves a single category by its ID.
 * The ID param is validated by Zod middleware.
 */
const getCategoryById = catchAsync(async (req, res, _next) => {
  const category = await categoryService.getCategoryById(req.params.id);
  if (!category)
    return _next(
      new ApiError(404, "Category not found with the specified ID.")
    );
  res.status(200).json(JSend.success({ category }));
});

/**
 * Deletes a category by its ID.
 * The ID param is validated by Zod middleware.
 */
const deleteCategory = catchAsync(async (req, res, _next) => {
  const deleted = await categoryService.deleteCategory(req.params.id);
  if (!deleted)
    return _next(
      new ApiError(404, "Category not found with the specified ID.")
    );
  res.status(204).send(); // No content on successful deletion
});

/**
 * Deletes all categories.
 */
const deleteAllCategories = catchAsync(async (req, res, _next) => {
  await categoryService.deleteAllCategories();
  res.status(204).send(); // No content on successful deletion
});

module.exports = {
  createCategory,
  getAllCategories,
  getCategoryById,
  updateCategory,
  deleteCategory,
  deleteAllCategories,
};
