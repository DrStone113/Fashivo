...
const updateProduct = catchAsync(async (req, res, next) => {
  ...
  if (req.file) {
    ...
    if (
      ...
      oldProduct.image_url.includes("/public/img/products/product-")
    ) {
  ...
  if (updatedProduct) {
    ...
    if (oldImagePath) {
      try {
        ...
        console.error(
          `Failed to delete old product image file: ${oldImagePath}`,
          err
        );
      }
    }
...
const deleteProduct = catchAsync(async (req, res, _next) => {
  ...
  if (product) {
    ...
    if (
      product.image_url &&
      product.image_url.includes("/public/img/products/product-")
    ) {
...
const deleteAllProducts = catchAsync(async (req, res, _next) => {
  ...
  for (const filePath of imagePathsToDelete) {
    try {
      ...
      console.error(
        `Failed to delete product image file during deleteAllProducts: ${filePath}`,
        err
      );
    }
  }
